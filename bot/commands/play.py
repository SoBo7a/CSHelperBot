...
def setup_play_commands(tree: app_commands.CommandTree, guild):    
    ...
    """
    @tree.command(description=translate("commands.play.description"))
    @app_commands.describe(action=translate("commands.play.choice_describe"))
    ...
    async def play(interaction: Interaction, action: str = ""):
        ...
        if action in ["subscribe", "unsubscribe"]:
            if action == "subscribe":
                ...
                await interaction.response.send_message(mention + translate("commands.play.sub_response"), ephemeral=True)
            elif action == "unsubscribe":
                delete_subscriptions(user_id)
                await interaction.response.send_message(mention + translate("commands.play.unsub_response"), ephemeral=True)
            return
        ...
        if user_id in cooldowns:
            ...
            if elapsed_time < 180:  # Cooldown period in seconds
                await interaction.response.send_message(mention + translate("commands.play.cooldown"), ephemeral=True)
                return
        ...
        if (user_id, username) not in subscriptions:
            await interaction.response.send_message(mention + translate("commands.play.not_subscribed"), ephemeral=True)
            return

        await interaction.response.send_message(translate("commands.play.inviting"), ephemeral=True)
            ...
            if user_id != sub_user_id:
                try:
                    ...
                    if cs2_channel:
                        ...
                        await sub_user.send(mention + translate("commands.play.priv_message") + invite.url)
                    else:
        ...
        if failed_mentions:
            await interaction.followup.send(mention + translate("commands.play.mentions") + ', '.join(failed_mentions))
        else:
            await interaction.followup.send(translate("commands.play.success"))
